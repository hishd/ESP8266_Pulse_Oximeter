...
void loop(){
  ...
  while (sensor.available())   {
    ...
    Serial.print(sensor.getFIFOIR());
    Serial.print(" ");
    Serial.println(sensor.getFIFORed());
#endif
...
void loop() {
  ...
  while (sensor.available())   {
      /*
       * 
       * Its important to swap the variable values since the current MAX30102 sensor has a swapped IR & RED LED.
       * 
       */
      ...
      if (numSamples == RFA_BUFFER_SIZE) {
        ...
        if(ch_spo2_valid && n_spo2 < SPO2_LOWER_BOUNDRY) {
          turnAllLEDOff();
          turnRedLEDOn();
        } else {
            if(!ch_spo2_valid){
